...
async function loadAndSetupVideos() {
    try {
        ...
        if (!response.ok) {
            ...
            showToastMessage(true, ['Failed to load videos. Please check your connection.']);
            return;
        }
        
        const data = await response.json();
        VIDEOS = data.results || data; // Handle both paginated and direct responses
        ...
        if (VIDEOS && VIDEOS.length > 0) {
            ...
            document.getElementById('videoDescription').innerHTML = 'No videos have been uploaded yet.';
        }
        
    } catch (error) {
        console.error('Error loading videos:', error);
        showToastMessage(true, ['Error loading videos. Please try again.']);
    }
}
...
/**
 * Loads and plays a video using HLS.js with MP4 fallback.
 * @param {number} id - The video ID.
...
function loadVideo(id, resolution) {
    ...
    // Find the video to get the direct MP4 URL
    ...
    if (Hls.isSupported()) {
        ...
        hls.on(Hls.Events.ERROR, (event, data) => {
            ...
            // If HLS fails with parsing error, fallback to direct MP4
            if (data.type === 'mediaError' && data.details === 'fragParsingError') {
                console.log("Falling back to direct MP4 playback");
                fallbackToMP4(id, video);
            }
        });
    } else {
        // Browser doesn't support HLS, use direct MP4
        fallbackToMP4(id, video);
    }
}

/**
 * Fallback to direct MP4 playback when HLS fails
 * @param {number} id - The video ID.
 * @param {Object} video - The video object.
 */
function fallbackToMP4(id, video) {
    if (hls) {
        hls.destroy();
        hls = null;
    }
    ...
    videoContainer.src = video.video_file;
    videoContainer.load();
...
function loadVideoInOverlay(id, resolution) {
    ...
    if (Hls.isSupported()) {
        ...
        overlayHls.on(Hls.Events.ERROR, (event, data) => {
            ...
            // If HLS fails with parsing error, fallback to direct MP4
            if (data.type === 'mediaError' && data.details === 'fragParsingError') {
                console.log("Falling back to direct MP4 playback in overlay");
                fallbackToMP4Overlay(id, video);
            }
        });
    } else {
        // Browser doesn't support HLS, use direct MP4
...
/**
 * Fallback to direct MP4 playback in overlay when HLS fails
 * @param {number} id - The video ID.
 * @param {Object} video - The video object.
 */
function fallbackToMP4Overlay(id, video) {
    if (overlayHls) {
        overlayHls.destroy();
        overlayHls = null;
    }
    ...
    // Use direct MP4 playback
    overlayVideoContainer.src = video.video_file;
    overlayVideoContainer.load();
